...
(function() {
  ...
  const sim1 = {
    boids: [
      { x: 0,   y: 0,   dx: 0.01, dy: 0.01 },
      { x: 0.1, y: 0.1, dx: 0.01, dy: 0.01 }
    ],
  ...
  // tickSimulation basic algorithm
  ...
  function tickSimulation(sim) {
    for (const referenceBoid of sim.boids) {
      ...
      if (Math.abs(totalRule1Move.dx) > 0.00001 || Math.abs(totalRule1Move.dy) > 0.00001) {
  ...
  function runTests() {
    ...
    rule1Suite.addTest({
      testFn: () => {
        ...
        return totalBoidRefDx < 0.000001;
      },
