...
function planMovements(sim) {
  for (const referenceBoid of sim.boids) {
    ...
    if (rule4Applied) {
      ...
      referenceBoid.dx += totalRule1Move.dx; referenceBoid.dy += totalRule1Move.dy;
    ...
    const computedHeading = Math.atan2(referenceBoid.dy, referenceBoid.dx);
    const headingChange = computedHeading - previousHeading;

    let desiredHeadingChange;
    if (Math.abs(headingChange) < Math.PI) {
      desiredHeadingChange = headingChange;
    } else if (headingChange < 0) {
      desiredHeadingChange = headingChange + (Math.PI * 2);
    } else {
      desiredHeadingChange = headingChange - (Math.PI * 2);
    }

    if (Math.abs(desiredHeadingChange) > sim.maxAngleChange) {
      const headingCorrection = sim.maxAngleChange / Math.abs(desiredHeadingChange);
      desiredHeadingChange *= headingCorrection;
    }

    const newHeading = previousHeading + desiredHeadingChange;
    referenceBoid.dx = newVelMagnitude * Math.cos(newHeading);
    referenceBoid.dy = newVelMagnitude * Math.sin(newHeading);
    //------------------
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const sim = {
    ...
    maxVelocityMagnitude: 10,
    minVelocityMagnitude: 1,
    maxAngleChange: Math.PI / 32,
    avoidanceFactor: 1.,
    cohesionFactor: 0.2,
    alignmentFactor: 0.5,
    margin: 80,
